...
Entrega: Marzo 24 2021


Descripcion:
La tienda de “Lagunitas” requiere de un sistema de cómputo que le permita llevar un inventario 
de los productos con los que cuenta cada una de sus sucursales. Por tal motivo, se te ha 
contratado para realizar dicho sistema. El sistema deberá mostrar el siguiente menú:
	* Registrar producto.
	* Listar productos.
	* Buscar producto.
	* Modificar producto.
	* Eliminar producto.
	* Salir.
La opción de registrar producto deberá solicitar al usuario un ID del producto a registrar, 
este valor debe ser único e irrepetible. Adicionalmente, se solicitarán los siguientes datos: 
Categoría del producto, nombre del producto, cantidad, costo unitario, fecha de registro y comentarios.
La opción de listar deberá mostrar en pantalla los datos de todos los productos registrados. 
Esta información deberá mostrarse de forma ordenada por categoría (ej., lácteos, cereales, 
farmacia, etc.)
La opción de buscar producto solicitará al usuario el ID del producto y una vez localizado deberá 
mostrar la información de dicho producto.
La opción de modificar producto solicitará al usuario el ID del producto, si el producto existe, 
los únicos datos que se pueden modificar son nombre del producto, categoría, precio unitario y 
la cantidad en existencia.
La opción de eliminar producto solicitará al usuario el ID del producto a eliminar, 
en caso de existir dicho producto deberá ser eliminado del sistema.
El programa debe terminar sólo cuando el usuario elija la opción de salir.


Conclusiones:
La resolucion de este problema fue un gran reto que aunque pareciera que es una aplicación
sencilla de los conocimientos y herramientas adquiridas en clase, la realidad es totalmente
diferente, ya que se expone la falta de comprension en cuanto al como trabajan y funcionan
cada elemento de Python, como las listas y los diccionarios. Cada uno de estos aunque
parezca sencillo de utilizar, si no se comprende bien, la aplicacion de los mismos
se traduce en problemas de aplicacion, ya que la falta de comprension de estos elementos
perjudica el avance en la resolucion del problema.
Despues de una ardua investigacion en equipo, de comparar resultados y aportar ideas
en cuanto a como resolver el problmema, se llegó a la solucion solicitada.
Este ejercicio en concreto aporto demasiado a nuestro conocimiento en cuanto a colecciones
de datos, ya que pudimos comprender de una mejor manera como trabajan estas, como aplicarlar
de manera correcta y como acceder a sus elementos.
"""
...
"""
Declaramos varias listas vacias, la lista "ids" almacena los id ingresados
...
while True:
    ...
    """
    Con este ciclo while y try and except controlamos que los errores cometidos por el usuario
    no terminen el programa
    """
    while True:
        try:
            print("Seleccione la opcion deseada", "1)Registrar Producto","2)Listar Productos", "3)Bucar Productos", "4)Modificar Productos", "5)Eliminar Productos", "6)Salir", sep="\n")
            select=int(input())
    ...
    if select ==1:
        ...
        y por ende no es necesario explicar todas.
        """
        while True:
            while True:
                try:
                    ...
                    if ID in ids: #comprobamos que el id ingresado no se encuentre dentro de la lista ids
                        ...
                        while True:
                            ...
                            if cat.isnumeric() == True or cat.isalpha() == False: 
                                """verificamos que 
                                que las entradas del usuario sean validas, solo pueden ser palabras
                                """
                        ...
                        while True: 
                            try:
                                ...
                                """
                                Agregamos los datos ingresados a nuestra lista inventario
    ...
    #Si la opcion es 2, listamos los productos
        ...
        if inventario != []:
            """
            iteramos sobre cada elemento de la lista categoria, despues se itera sobre la
